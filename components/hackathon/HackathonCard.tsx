  ...
  return (
    <Link href={`/hackathons/${hackathon.slug}`} className="block h-full outline-none">
      <div className={cn(
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4 z-10 w-full">
        ...
        <h3 className="font-sans font-bold text-[--text-primary] text-sm leading-tight mb-5 group-hover:text-glow transition-all flex-1 z-10 line-clamp-2 break-keep">
          {hackathon.title}
        </h3>

        <div className="bg-[#000]/30 rounded-sm p-3 border border-[--border]/50 mb-4 z-10 group-hover:bg-[#000]/50 transition-colors mt-auto">
          <div className="grid grid-cols-1 gap-1.5">
            <div className="flex items-center gap-2 font-mono text-xs whitespace-nowrap">
              ...
              <span className="text-[--text-secondary] w-14 text-right">START</span>
              <span className="text-[--border] mx-1">|</span>
              <span className="text-[--text-primary] truncate">{formatDate(hackathon.period.startAt)}</span>
            </div>
            <div className="flex items-center gap-2 font-mono text-xs whitespace-nowrap">
              <Calendar size={12} className="text-[--text-secondary]" />
              <span className="text-[--text-secondary] w-14 text-right">END</span>
              <span className="text-[--border] mx-1">|</span>
